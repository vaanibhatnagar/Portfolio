...
export default function ResumeSection() {
  ...
  const getExperienceDotColor = (index: number) => {
    return index === 0 ? "bg-primary" : "bg-accent";
  };
            ...
            <div className="space-y-8">
              {experiences.map((experience, index) => (
                <div key={experience.id} className="relative pl-8 border-l-2 border-blue-200 dark:border-blue-800">
                  <div className={`absolute -left-2 top-0 w-4 h-4 rounded-full ${getExperienceDotColor(index)}`}></div>
